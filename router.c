...
static bool router_setup_hashgrid(router_t *router) {
    ...
    coords = (coord_t *) malloc(sizeof(coord_t) * router->tdata->n_stop_points);
    ...
    do {
        ...
        coord_from_latlon(coords + i_sp,
                          router->tdata->stop_point_coords + i_sp);
    } while(i_sp);
...
static void apply_transfers (router_t *router, router_request_t *req,
    ...
    for (sp_index_from = bitset_next_set_bit (router->updated_stop_points, 0);
        ...
        /* First apply a transfer from the stop_point to itself,
        ...
        if (states_time[sp_index_from] == router->best_time[sp_index_from]) {
            ...
            /* assert (router->best_time[sp_index_from] == time_from); */
        ...
        for ( ; tr < tr_end ; ++tr) {
            ...
            if (router->best_time[sp_index_to] == UNREACHED ||
                ...
                states_walk_from[sp_index_to] = sp_index_from;
    ...
    for (sp_index_from = bitset_next_set_bit (router->updated_walk_stop_points, 0);
        ...
        flag_journey_patterns_for_stop_point(router, req, sp_index_from);
    }
...
write_state(router_t *router, router_request_t *req,
            ...
            spidx_t sp_index, uint16_t jpp_offset, rtime_t time,
