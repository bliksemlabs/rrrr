...
static void tdata_realtime_free_trip_index (tdata_t *tdata, uint32_t trip_index) {
    if (tdata->vj_stoptimes[trip_index]) {
        free(tdata->vj_stoptimes[trip_index]);
        tdata->vj_stoptimes[trip_index] = NULL;
        /* TODO: also free a forked journey_pattern and the reference to it */
        /* TODO: restore original validity
         * TODO: test if below works
         */
        tdata->vj_active[trip_index] = tdata->vj_active_orig[trip_index];
/*        memcpy (&(tdata->vj_active[trip_index]),
                &(tdata->vj_active_orig[trip_index]),
                sizeof(calendar_t));*/
    }
}
...
static uint32_t tdata_new_journey_pattern(tdata_t *tdata, char *trip_ids,
    ...
    strncpy(&tdata->vj_ids[tdata->n_vjs * tdata->vj_ids_width],
            trip_ids, tdata->vj_ids_width * n_vjs);
    ...
    for (i_vj = 0; i_vj < n_vjs; ++i_vj) {
        ...
        radixtree_insert(tdata->lineid_index,
                         &trip_ids[i_vj * tdata->vj_ids_width],
    ...
    for (i_stu = 0;
        ...
        if (rt_stop_time_update->schedule_relationship != TRANSIT_REALTIME__TRIP_UPDATE__STOP_TIME_UPDATE__SCHEDULE_RELATIONSHIP__SKIPPED) {
            ...
            if (stop_id) {
                ...
                tdata_apply_gtfsrt_time (rt_stop_time_update, &tdata->vj_stoptimes[trip_index][rs]);
...
static void tdata_realtime_changed_journey_pattern(tdata_t *tdata, uint32_t vj_index, int16_t cal_day, uint16_t n_stops, TransitRealtime__TripUpdate *rt_trip_update) {
    ...
    if (jp_new == NULL) {
        ...
        vehicle_journey_t  *trip = tdata->vjs + vj_index;
        uint16_t i_vj;
        ...
        for (i_vj = 0; i_vj < 1; ++i_vj) {
            tdata->vjs[vj_index].trip_attributes = trip->trip_attributes;
...
static void tdata_realtime_apply_tripupdates (tdata_t *tdata, uint32_t trip_index, TransitRealtime__TripUpdate *rt_trip_update) {
    ...
    jp = tdata->journey_patterns + tdata->vjs_in_journey_pattern[trip_index];
    vj = tdata->vjs + trip_index;

    /* Normal case: at least one SCHEDULED or some NO_DATA
     * stops have been observed
     */
    if (tdata->vj_stoptimes[trip_index] == NULL) {
        /* If the expanded timetable does not contain an entry yet, we are creating one */
        tdata->vj_stoptimes[trip_index] = (stoptime_t *) malloc(sizeof(stoptime_t) * jp->n_stops);
    }
    ...
    for (rs = 0; rs < jp->n_stops; ++rs) {
        tdata->vj_stoptimes[trip_index][rs].arrival   = vj->begin_time + trip_times[rs].arrival;
        tdata->vj_stoptimes[trip_index][rs].departure = vj->begin_time + trip_times[rs].departure;
    }
                    ...
                    if (rt_stop_time_update_prev) {
                        if (rt_stop_time_update_prev->schedule_relationship == TRANSIT_REALTIME__TRIP_UPDATE__STOP_TIME_UPDATE__SCHEDULE_RELATIONSHIP__SCHEDULED &&
                            ...
                            for (; propagate < rs; ++propagate) {
                                tdata->vj_stoptimes[trip_index][propagate].arrival   += SEC_TO_RTIME(rt_stop_time_update_prev->departure->delay);
                                tdata->vj_stoptimes[trip_index][propagate].departure += SEC_TO_RTIME(rt_stop_time_update_prev->departure->delay);
                            }
                        }
                    }
                    tdata_apply_gtfsrt_time (rt_stop_time_update, &tdata->vj_stoptimes[trip_index][rs]);
    ...
    if (rt_stop_time_update->schedule_relationship == TRANSIT_REALTIME__TRIP_UPDATE__STOP_TIME_UPDATE__SCHEDULE_RELATIONSHIP__SCHEDULED &&
        ...
        for (; rs < jp->n_stops; ++rs) {
            tdata->vj_stoptimes[trip_index][rs].arrival   += SEC_TO_RTIME(rt_stop_time_update->departure->delay);
            tdata->vj_stoptimes[trip_index][rs].departure += SEC_TO_RTIME(rt_stop_time_update->departure->delay);
        }
    }
}
...
bool tdata_alloc_expanded(tdata_t *td) {
    ...
    for (i_vj = 0; i_vj < td->n_journey_patterns; ++i_vj) {
        uint32_t i_trip;
        for (i_trip = 0; i_trip < td->journey_patterns[i_vj].n_vjs; ++i_trip) {
            td->vjs_in_journey_pattern[td->journey_patterns[i_vj].vj_ids_offset + i_trip] =
                    i_vj;
        }
    }
    ...
    for (e = 0; e < msg->n_entity; ++e) {
        ...
        if (rt_trip_update) {
            ...
            if (rt_trip->schedule_relationship ==
                ...
                tdata->vj_active[trip_index] &= ~(1 << cal_day);
...
void tdata_clear_gtfsrt (tdata_t *tdata) {
    ...
    for (i_vj_index = 0; i_vj_index < tdata->n_vjs; ++i_vj_index) {
        tdata_realtime_free_trip_index (tdata, i_vj_index);
    }
