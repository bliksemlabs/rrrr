...
bool router_request_reverse(router_t *router, router_request_t *req) {
...

    {
        ...
        for (r = 0; r <= max_transfers; ++r) {
            if (router->states_walk_time[router->states_pointer[r * router->tdata->n_stops + best_stop_index]] != UNREACHED) {
                round = r;
    ...
    req->time = router->states_walk_time[router->states_pointer[round * router->tdata->n_stops +
                               best_stop_index]];
    #if 0
